...
\texttt{outputs/verify-cut-independent.py} and the independently written
\texttt{outputs/verify-independent.py} recompute the identities, the rational
margins and finite instances; both use exact arithmetic only.
\texttt{formal/Scalar.lean} carries a compiled Lean 4 certificate for the
scalar optimization of Section 4 and for the polynomial identities of
Section 2, conditional on scalar hypotheses supplied as arguments.
...
underlying the attribution in Section 2 is recorded in \texttt{LITERATURE.md};
...
recorded in \texttt{LITERATURE.md}. The author takes responsibility for the
